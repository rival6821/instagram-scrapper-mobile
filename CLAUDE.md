...
**Failure modes are typed as string status codes**, not exceptions, and propagate from `fetch_user_posts()` up through `run_scraper()` to both the cron log and Telegram notifications: `SUCCESS`, `FAIL_SESSION` (401 / login-required payload → triggers `notify_session_expired`), `FAIL_RATE_LIMIT` (429 → triggers `notify_rate_limit`), `FAIL_NETWORK` (timeout/connection error, retried with exponential backoff up to `max_retries`), `FAIL_UNKNOWN` (also covers a non-401/429/200 status code, after backoff+retries are exhausted), `FAIL_CONFIG`, `FAIL_LOCKED` (another run already holds `scraper.lock`, see below). When adding a new failure path, follow this pattern rather than raising — callers (bot, cron log formatting) branch on these strings.

**Single-instance locking lives inside `run_scraper()`, not in the crontab line.** It opens `LOCK_FILE_PATH` (`scraper.lock`) and takes a non-blocking `fcntl.flock` before doing any work, releasing it in a `finally` when the run ends. Because both entry points — cron's `python scraper.py` and the bot's `/run` (`cmd_run` → `run_scraper()` via `run_in_executor`) — go through this same function, the lock is what actually prevents a cron tick and a manually-triggered `/run` from racing on `session.json`/the SQLite DB; if it can't be acquired, the run returns `FAIL_LOCKED` immediately instead of blocking. Do not additionally wrap the crontab command in a shell-level `flock` on the same file — a second, independent open-file-description lock on `scraper.lock` from the parent shell would just make every normal cron run fail to acquire its own internal lock.

**Telegram messages that embed dynamic content must be HTML-escaped.** All bot replies and notifier messages are sent with `parse_mode=HTML`. Anything that isn't a fixed literal — error strings, log tails, exception messages, scraped usernames — must go through `html.escape()` before being interpolated, otherwise a stray `<`/`>`/`&` (extremely common in Python tracebacks, e.g. `in <module>`, or in an HTML page Instagram returned instead of JSON) breaks Telegram's HTML parser and silently drops the message. Follow the existing `html.escape(str(x))` pattern in `telegram_bot.py`/`telegram_notifier.py` when adding new dynamic content to a message.
